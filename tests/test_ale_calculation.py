...
from .utils import DummyInteractionModel, DummyLinearModel
...
def test_linear():
    ...
    for column in X.columns:
        quantiles, ale = first_order_ale_quant(
            DummyLinearModel().predict, X, column, 10
        )
    ...
    ale_second_order = second_order_ale_quant(
        DummyLinearModel().predict, X, X.columns, 21
    )[1]
...
def test_interaction():
    ...
    quantiles_list, ale = second_order_ale_quant(
        DummyInteractionModel().predict, X, X.columns, nbins
    )[:2]
    ...
    # interaction predictor. The error here (which we require to be below `atol`
